...
class AppleMapsService {
	...
	static settingsLoadedTransform(settings = {}) {
		...
		for (const item in settings) {
			if (settings[item]) {
				switch (item) {
					case "center":
						newSettings[item] = new window.mapkit.Coordinate(
							settings[item].latitude,
							settings[item].longitude
						);
						break;
					case "region":
						const regionCenter = new window.mapkit.Coordinate(
							settings[item].center.latitude,
							settings[item].center.longitude
						);
						if (settings[item].span) {
							const regionSpan = new window.mapkit.CoordinateSpan(
								settings[item].span.from,
								settings[item].span.to
							);
							newSettings[item] =
						...
						newSettings[item] = new window.mapkit.CoordinateRegion(
							regionCenter
						);
						break;
					case "padding":
						newSettings[item] = new window.mapkit.Padding(
							settings[item]
						);
						break;
					default:
	...
	createMap(element) {
		...
		const index = this.maps.push(
			new window.mapkit.Map(
				element,
				AppleMapsService.settingsLoadedTransform(options.settings)
			)
		);
		const object = {
			key: index - 1,
			...
			addEventListener: (type, listener, context) => {
				...
				return this.maps[index - 1].addEventListener(
					type,
					listener,
					context
				);
			...
			setCenterAnimated: (latitude, longitude, animate = true) => {
				this.maps[index - 1].setCenterAnimated(
			...
			setRegionAnimated: (center, span = null, animate = true) => {
				...
				if (span) {
					...
					this.maps[index - 1].setRegionAnimated(
						new window.mapkit.CoordinateRegion(
							regionCenter,
							regionSpan
						),
				...
				this.maps[index - 1].setRegionAnimated(
					new window.mapkit.CoordinateRegion(regionCenter),
					animate
				);
			},
			getRotation: () => {
			...
			setCameraDistanceAnimated: (distance, animate = true) => {
				this.maps[index - 1].setCameraDistanceAnimated(
					distance,
					animate
				);
			},
			getCameraZoomRange: () => {
				const { minCameraDistance, maxCameraDistance } =
					this.maps[index - 1].cameraZoomRange;
			...
			setCameraZoomRangeAnimated: (
				...
				this.maps[index - 1].setCameraZoomRangeAnimated(
					new window.mapkit.CameraZoomRange(
						minCameraDistance,
						maxCameraDistance
					),
					animate
			...
			setPadding: (padding) => {
				this.maps[index - 1].padding = new window.mapkit.Padding(
					padding
				);
			},
			...
			showItems: (items, mapShowItemOptions = {}) => {
				...
				if (mapShowItemOptions.span) {
					// @ts-ignore
					passingOptions.minimumSpan =
						new window.mapkit.CoordinateSpan(
							mapShowItemOptions.span.from,
							mapShowItemOptions.span.to
						);
				}
				if (mapShowItemOptions.padding) {
					// @ts-ignore
					passingOptions.padding = new window.mapkit.Padding(
						mapShowItemOptions.padding
					);
				}
				...
				if (item.previousValue !== item.currentValue) {
					switch (item.key) {
						case "colorScheme":
							...
							break;
						case "center":
							this.maps[key].setCenterAnimated(
								new window.mapkit.Coordinate(
									item.currentValue.latitude,
									item.currentValue.longitude
								),
								true
							);
							break;
						case "region":
							...
							const regionCenter = new window.mapkit.Coordinate(
								item.currentValue.center.latitude,
								item.currentValue.center.longitude
							);
							if (item.currentValue.span) {
								const regionSpan =
									new window.mapkit.CoordinateSpan(
										item.currentValue.span.from,
										item.currentValue.span.to
									);
								this.maps[key].setRegionAnimated(
									new window.mapkit.CoordinateRegion(
										regionCenter,
										regionSpan
									)
							...
							break;
						case "padding":
							this.maps[key] = new window.mapkit.Padding(
								item.currentValue
							);
							break;
						default:
...
class AppleMapsGeocoderService {
	...
	reverseLookup(latitude, longitude, callback, options = {}) {
		...
		return this.geocoderInstance.reverseLookup(
			new window.mapkit.Coordinate(latitude, longitude),
			callback,
			options
		);
	...
	reverseLookupPromised(latitude, longitude, options = {}) {
		...
		return new Promise((resolve, reject) => {
			this.geocoderInstance.reverseLookup(
				new window.mapkit.Coordinate(latitude, longitude),
				(err, result) => {
					if (err) {
						reject(err);
...
class AppleMapsSearchService {
	...
	autocompletePromised(query, options) {
		...
		return new Promise((resolve, reject) => {
			this.searchInstance.autocomplete(
				query,
				(error, data) => {
					if (error) {
						reject(error);
...
class AppleMapkitComponent {
	...
	static {
		this.ɵfac = i0.ɵɵngDeclareFactory({
			...
			version: "17.1.3",
			ngImport: i0,
			type: AppleMapkitComponent,
			deps: [
				{ token: i0.KeyValueDiffers },
				{ token: AppleMapsService },
	...
	static {
		this.ɵcmp = i0.ɵɵngDeclareComponent({
			...
			outputs: { onLoaded: "onLoaded" },
			queries: [
				{
					propertyName: "annotations",
					predicate: ["apple-mapkit-annotation"],
					descendants: true,
				},
			],
			ngImport: i0,
			...
			styles: [
				".ngx-apple-mapkit{height:100%;position:relative}.ngx-apple-mapkit .ngx-apple-mapkit__map{inset:0;position:absolute}\n",
			],
		});
	}
}
...
class AppleMapkitAnnotationComponent {
	set latitude(value) {
		if (this.annotation) {
			this.annotation.coordinate = new window.mapkit.Coordinate(
				value,
				this._longitude
			);
		}
	...
	initAnnotation(options = {}) {
		...
		// @ts-ignore
		if (
			!this.ref.nativeElement.children.length ||
			(transformedOptions && transformedOptions.calloutEnabled === false)
		) {
	...
	transformOptions(options) {
		...
		for (const item in options) {
			if (options[item]) {
				transformedOptions[item] = transformedOptions[item] =
					this.transformOption(item, options[item]);
			}
		}
		return transformedOptions;
	}
	createAnnotation(latitude, longitude, options = {}) {
		this.annotation = new window.mapkit.MarkerAnnotation(
			new window.mapkit.Coordinate(latitude, longitude),
			options
		);
	...
	optionsChanged(changes) {
		changes.forEachItem((item) => {
			if (item.previousValue !== item.currentValue) {
				this.annotation[item.key] = this.transformOption(
					item.key,
					item.currentValue
				);
			}
	...
	ngOnDestroy() {
		this.appleMapsService.maps[this.parentKey].removeAnnotation(
			this.annotation
		);
	}
	static {
		this.ɵfac = i0.ɵɵngDeclareFactory({
			...
			version: "17.1.3",
			ngImport: i0,
			type: AppleMapkitAnnotationComponent,
			deps: [
				{ token: AppleMapsService },
				{ token: i0.KeyValueDiffers },
				{ token: i0.ElementRef },
				{ token: i0.Renderer2 },
				{ token: AppleMapkitComponent },
			],
			target: i0.ɵɵFactoryTarget.Component,
		});
	}
	static {
		this.ɵcmp = i0.ɵɵngDeclareComponent({
			...
			outputs: { onSelect: "onSelect", onDeselect: "onDeselect" },
			ngImport: i0,
			...
			args: [
				{
					...
					template: "<ng-content></ng-content>\n",
					styles: [
						".ngx-apple-mapkit__map-landmark{background:#fff}\n",
					],
				},
			],
		},
...
class AppleMapsModule {
	...
	static {
		this.ɵmod = i0.ɵɵngDeclareNgModule({
			...
			ngImport: i0,
			type: AppleMapsModule,
			bootstrap: [AppleMapkitAnnotationComponent],
			declarations: [
				AppleMapkitComponent,
				AppleMapkitAnnotationComponent,
			],
			imports: [CommonModule],
			exports: [AppleMapkitComponent, AppleMapkitAnnotationComponent],
		});
	}
	static {
